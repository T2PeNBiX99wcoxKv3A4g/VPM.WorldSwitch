...
namespace VRC.PackageManagement.Resolver
{
    ...
    {
        private const string HAS_REFRESHED_KEY = "VRC.PackageManagement.Resolver.Refreshed";
        ...
        private static readonly Color _colorPositive = Color.green;
        private static readonly Color _colorNegative = new(1, 0.3f, 0.3f);
        ...
        private void CreateGUI()
        {
            ...
            {
                _createButton = new(Resolver.CreateManifest)
                {
                    text = "Create",
            ...
            {
                _resolveButton = new(Resolver.ResolveManifest)
                {
                    text = "Resolve All",
            ...
            _manifestLabel = new("Required Packages")
            {
                name = "manifest-header"
            };
            _manifestInfo.Add(_manifestLabel);
            _manifestInfoText = new();
        ...
        {
            var wnd = GetWindow<ResolverWindow>();
            wnd.titleContent = new("Package Resolver");
        }
        ...
        {
            var havePackage = package != null;

            // Table Row
            VisualElement row = new Box
            {
                name = "package-row"
            };
            VisualElement column1 = new Box
            {
                name = "package-box"
            };
            VisualElement column2 = new Box
            {
                name = "package-box"
            };
            VisualElement column3 = new Box
            {
                name = "package-box"
            };
            VisualElement column4 = new Box
            {
                name = "package-box"
            };

            column1.style.minWidth = 200;
            column1.style.width = new(new Length(40, LengthUnit.Percent));
            column2.style.minWidth = 100;
            column2.style.width = new(new Length(19f, LengthUnit.Percent));
            column3.style.minWidth = 100;
            column3.style.width = new(new Length(19f, LengthUnit.Percent));
            column4.style.minWidth = 100;
            column4.style.width = new(new Length(19f, LengthUnit.Percent));
            ...
            if (!havePackage)
            {
                var missingText = new TextElement
                {
                    text = "MISSING"
                };
            ...
            {
                var package = Repos.GetPackageWithVersionMatch(id, popupField.value);
                    ...
                    Version.TryParse(popupField.value, out var newVersion))
                {
                    ...
                    for (var v = 0; v < affectedPackages.Count; v++)
