...
namespace VRC.PackageManagement.PackageMaker
{
    public class PackageMakerWindow : EditorWindow
    {
        ...
        private readonly Regex packageIdRegex = new("[^a-z0-9.]");
        ...
        {
            var wnd = GetWindow<PackageMakerWindow>();
            wnd.titleContent = new("Package Maker");
        }
        ...
        private static void ExportAsUnityPackage()
        {
            ...
            var exportFilename = new StringBuilder("exported");
            foreach (var guid in Selection.assetGUIDs)
            {
                var selectedFolder = AssetDatabase.GUIDToAssetPath(guid);
        ...
        {
            _targetVRCPackageField = new("Related VRChat Package", VRCPackageEnum.None);
        ...
        {
            _actionButton = new(OnActionButtonPressed)
            {
        ...
        {
            var result = EditorUtility.DisplayDialog("One-Way Conversion",
            ...
            if (result)
            {
                var newPackageFolderPath = Path.Combine(_projectDir, "Packages", _windowData.packageID);
        ...
        private VisualElement CreatePackageIDElement()
        {
            ...
            _packageIDField = new("Package ID", 255, false, false, '*');
        ...
        private VisualElement CreateAuthorElement()
        {
            ...
            _authorNameField = new("Author Name");
            _authorEmailField = new("Author Email");
            _authorUrlField = new("Author URL (optional)");
        ...
        private void OnPackageIDChanged(ChangeEvent<string> evt)
        {
            ...
            {
                var newId = packageIdRegex.Replace(evt.newValue, "-");
        ...
        private VisualElement CreateTargetFolderElement()
        {
            ...
            _targetAssetFolderField = new("Target Folder");
            ...
            targetFolderBox.Add(new Label("Drag and Drop an Assets Folder to Convert Above")
            {
                name = "description"
            });
        ...
        private bool StringIsValidAssetFolder(string targetFolder) =>
            !string.IsNullOrWhiteSpace(targetFolder) && AssetDatabase.IsValidFolder(targetFolder);

        private void OnTargetAssetFolderValueChanged(ChangeEvent<string> evt)
        {
            var targetFolder = evt.newValue;
        ...
        private void DoMigration(string corePath, string targetDir)
        {
            ...
            var parentDir = new DirectoryInfo(targetDir)?.Parent.FullName;
        ...
        private static IEnumerable<string> GetAllFiles(string path)
        {
            ...
            return Directory.EnumerateFiles(path, "*.*", SearchOption.AllDirectories)
                .Where(s => excludedPaths.All(entry => !s.Contains(entry))
                );
        }
        ...
        public static void MoveFilesToPackageDir(List<string> files, string pathBase, string targetDir)
        {
            ...
            for (var i = 0; i < files.Count; i++)
                try
                {
                    ...
                    var file = files[i];
                    var simplifiedPath = file.Replace($"{pathBase}\\", "");
                    ...
                    var targetEnclosingDir = Path.GetDirectoryName(dest);
